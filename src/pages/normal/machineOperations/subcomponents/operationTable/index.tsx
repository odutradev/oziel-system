...
const OperationTable = ({ operations, operators, fleets, onEdit, onDelete, onStatusChange }: OperationTableProps) => {
    const columns: TableColumn<MachineOperationModelType>[] = [
        ...
        { key: "operator", label: "Operador", render: (row) => typeof row.operator === "object" ? (row.operator as any).name : (operators.find(o => o._id === row.operator)?.name || "-") },
        { key: "fleet", label: "Frota", render: (row) => typeof row.fleet === "object" ? (row.fleet as any).name : (fleets.find(f => f._id === row.fleet)?.name || "-") },
    ...
    const rowActions: RowAction<MachineOperationModelType>[] = [
        { label: "Concluir", icon: <CheckCircle color="success" fontSize="small" />, onClick: (row) => row._id && onStatusChange(row._id, "COMPLETED" as any), show: (row) => row.status !== "COMPLETED", isInline: true },
